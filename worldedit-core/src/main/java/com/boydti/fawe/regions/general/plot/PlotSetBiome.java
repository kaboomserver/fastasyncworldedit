...
public class PlotSetBiome extends Command {
    ...
    private BiomeRegistry getBiomeRegistry() {
        return WorldEdit.getInstance().getPlatformManager().queryCapability(Capability.GAME_HOOKS).getRegistries().getBiomeRegistry();
    }
    ...
    public CompletableFuture<Boolean> execute(final PlotPlayer player, String[] args, RunnableVal3<Command, Runnable, Runnable> confirm, RunnableVal2<Command, CommandResult> whenDone) throws CommandException {
        ...
        final @NotNull Set<CuboidRegion> regions = plot.getRegions();
        ...
        if (biome == null) {
            String biomes = StringMan.join(knownBiomes.stream().map(BiomeType::getId
            ).collect(Collectors.toList()), Captions.BLOCK_LIST_SEPARATOR.getTranslated());
